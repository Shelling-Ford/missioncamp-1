...
def check_session(camp, logintype, idx):
    return True if not 'type' in session or session['type'] != logintype or not 'idx' in session or session['idx'] != int(idx) else False
...
def reg_individual_proc(camp):
    ...
    if check > 0:
        ...
        regIndividual(campidx, formData)
...
def login_proc(camp):
    ...
    if logintype == u'개인':
        ...
        if loginCheckGroupid(campidx, userid, pwd):
            ...
            session['type'] = u'단체'
            session['idx'] = idx
            return redirect(url_for('show_group', camp=camp, idx=idx))
        else:
            flash(u'아이디 또는 비밀번호가 잘못되었습니다.')
            return redirect(url_for('login', camp=camp))
    else:
...
def reg_group_proc(camp):
    ...
    if check > 0:
        ...
        regGroup(campidx, formData)

        flash(u'신청이 완료되었습니다.')
        return redirect(url_for('home'))

# 단체신청 조회
@context.route('/<camp>/group/<idx>')
def show_group(camp, idx):
    if check_session(camp, u'단체', idx):
        flash(u'로그아웃 되었습니다. 다시 로그인하세요')
        return redirect(url_for('login', camp=camp))
...
# 단체 멤버 추가
@context.route('/<camp>/group/<idx>/member/add')
def member_add(camp, idx):
    if check_session(camp, u'단체', idx):
        flash(u'로그아웃 되었습니다. 다시 로그인하세요')
        return redirect(url_for('login', camp=camp))
    check_campcode(camp)
...
# 단체 멤버 추가 적용
@context.route('/<camp>/group/<idx>/member/add', methods=['POST'])
def member_add_proc(camp, idx):
    if check_session(camp, u'단체', idx):
        flash(u'로그아웃 되었습니다. 다시 로그인하세요')
        return redirect(url_for('login', camp=camp))
    check_campcode(camp)
...
# 단체 멤버 수정
@context.route('/<camp>/group/<idx>/member/<member_idx>/edit')
def member_edit(camp, idx, member_idx):
    if check_session(camp, u'단체', idx):
        flash(u'로그아웃 되었습니다. 다시 로그인하세요')
        return redirect(url_for('login', camp=camp))
    check_campcode(camp)
