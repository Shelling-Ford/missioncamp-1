...
def reg_individual():
    ...
    return render_template('individual/add.html', camp='cmc', campidx=campidx,
...
def show_individual():
    ...
    return render_template('individual/show.html', camp='cmc', member=member, area_name=area_name)
...
def edit_individual():
    ...
    area_list = getAreaList('cmc') # form에 들어갈 지부 목록
    date_select_list = getDateSelectList() # form 생년월일에 들어갈 날자 목록
    return render_template('individual/edit.html', camp='cmc', campidx=campidx, member=member, membership=member['membership'],
        area_list=area_list, date_select_list=date_select_list, group_yn=False)
...
def cancel_individual():
    ...
    return render_template('individual/cancel.html')
...
def reg_group():
    campidx = getCampIdx('cmc')
    return render_template('group/add.html', camp_idx=campidx, area_list=getAreaList('cmc'))
...
def show_group():
    ...
    return render_template('group/show.html', group=group, area=getAreaName(group['area_idx']), member_list=member_list)
...
def edit_group():
    ...
    campidx = getCampIdx('cmc')
    area_list = getAreaList('cmc') # form에 들어갈 지부 목록
    return render_template('group/edit.html', group=group, area_list=area_list, campidx=campidx)
...
def cancel_group():
    if check_session(u'단체'):
        flash(u'로그아웃 되었습니다. 다시 로그인하세요')
        return redirect(url_for('login'))
    return render_template('group/cancel.html')
