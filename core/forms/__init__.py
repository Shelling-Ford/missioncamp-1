...
class GroupForm(Form):
    ...
    def set_camp(self, camp):
        ...
        if camp == 'youth':
            self.grouptype = RadioField('단체 유형')
...
class RegistrationForm(Form):
    ...
    # membership
    vision_yn = HiddenField()
    # membership
    training = MultiCheckboxField('인터콥 훈련여부')
    ...
    def set_camp(self, camp):
        ...
        if camp == 'cmc':
            self.campus = StringField('캠퍼스')
            self.major = StringField('전공')

        if camp in ['cmc', 'cbtj']:
            self.vision_yn = RadioField('비전스쿨 수료여부', choices=[(1, '예'), (0, '아니오')])
            self.job = SelectField('직업/직종', choices=[(i, i) for i in form_config.JOBS])

    def set_group_mode(self):
        self.group_yn = True
        self.userid = HiddenField()
        self.pwd = HiddenField()
        self.pwd2 = HiddenField()
        self.area_idx = HiddenField()
    ...
    def populate_membership(self, member_idx):
        ...
        for membership_field in membership_fields:
            if self.getattr(membership_field).data is not None:
                membership = Membership()
                membership.camp_idx = camp_idx
                membership.member_idx = member_idx
                membership.key = membership_field
                membership.value = self.getattr(membership_field).data
    ...
    def insert(self, group_idx=None):
        member = Member()
