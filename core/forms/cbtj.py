...
# 참가구분
persontypes = ['청년', '스탭']  # 상반기 선캠
# persontypes = ['청년', '대학생', '스탭']  # 하반기 선캠

# 인터콥 훈련여부
trainings = [
    ('training1', '비전스쿨'), ('training2', 'BTJ스쿨'), ('training3', '월드미션'), ('training4', '선교캠프'), ('training5', 'MIT'),
    ('training6', 'FO'), ('training7', 'C-BTJ'), ('training8', 'Y-Tentmaker'), ('training9', 'SM'), ('training10', '기타'), ('none', '없음')
]


class MemberCommon():
    def populate_membership(self, member_idx):
        if self.job.data not in [None, '', 'None']:
            job = Membership()
            job.camp_idx = Camp.get_idx('cbtj')
        ...
        if self.training.data not in [None, '', 'None']:
            for training in self.training.data:
                ...
                t_membership.camp_idx = Camp.get_idx('cbtj')
...
class RegistrationForm(Form, MemberCommon):
    ...
    name = StringField('이름')
    area_idx = SelectField('지부', choices=Area.get_list('cbtj'))
    ...
    date_of_arrival = SelectField('캠프오는날', choices=Camp.get_date_list(Camp.get_idx('cbtj')))
    date_of_leave = SelectField('집에가는날', choices=Camp.get_date_list(Camp.get_idx('cbtj')))
    ...
    def insert(self):
        member = Member()
        member.camp_idx = Camp.get_idx('cbtj')
...
class GroupMemberRegForm(Form, MemberCommon):
    ...
    def set_member_data(self, member):
        ...
        self.date_of_leave.data = member.date_of_leave
        self.newcomer_yn.data = member.newcomer_yn
        self.vision_yn.data = membership_data.get('vision_yn')
        self.training.data = membership_data.get('training')
        self.language.data = member.language
