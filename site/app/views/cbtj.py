...
def reg_individual(camp):
    ...
    return render_template('cbtj/%s/individual/add.html' % camp, camp=camp, campidx=campidx,
...
def edit_individual(camp, idx):
    ...
    area_list = getAreaList('cbtj') # form에 들어갈 지부 목록
    date_select_list = getDateSelectList() # form 생년월일에 들어갈 날자 목록
    return render_template('cbtj/%s/individual/edit.html' % camp, camp=camp, campidx=campidx, member=member, membership=member['membership'],
        area_list=area_list, date_select_list=date_select_list, group_yn=False)
...
def reg_group(camp):
    check_campcode(camp) # camp code  가 cbtj 또는 cbtj2가 아닐 경우 오류 페이지로 리다이렉션
    campidx = getCampIdx(camp)
    return render_template('cbtj/%s/group/add.html' % camp, camp_idx=campidx, area_list=getAreaList('cbtj'))
...
def edit_group(camp):
    ...
    campidx = getCampIdx(camp)
    area_list = getAreaList('cbtj') # form에 들어갈 지부 목록
    return render_template('cbtj/%s/group/edit.html' % camp, group=group, area_list=area_list, campidx=campidx)
