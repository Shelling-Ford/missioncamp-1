...
def room_check():
    if request.method == 'POST':
        ...
        if logintype == u'개인':
            try:
                member = db.session.query(Member).filter(Member.contact == contact, Member.name == name, Member.cancel_yn == 0, or_(Member.camp_idx == Camp.get_idx('kids'))).one()
                member_list = [member]
                group_name = None
            ...
            try:
                group = db.session.query(Group).filter(Group.leadercontact == contact, Group.leadername == name, Group.cancel_yn == 0, or_(Group.camp_idx == Camp.get_idx('kids'))).one()
...
def member_add_proc():
    ...
    campidx = Camp.get_idx('kids')
    formData = getIndividualFormData()
    Member.insert(campidx, formData, idx, getMembershipDataList(campidx, formData))
