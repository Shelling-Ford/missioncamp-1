...
from core.models import Promotion, Member, Group, Camp
...
def room_check():
    if request.method == 'POST':
        ...
        if logintype == u'개인':
            try:
                member = db.session.query(Member).filter(Member.contact == contact, Member.name == name, Member.cancel_yn == 0, or_(Member.camp_idx == Camp.get_idx('youth'))).one()
                member_list = [member]
                group_name = None
            except NoResultFound:
                return render_template('youth/room-check-result.html', msg=u'접수된 신청 정보가 없습니다^^ 이름과 연락처를 확인해주세요', name=name)
            except MultipleResultsFound:
                return render_template('youth/room-check-result.html', msg=u'중복신청자입니다. 로비의 숙소배치팀에 문의해주세요.', name=name)
        elif logintype == u'단체':
            try:
                group = db.session.query(Group).filter(Group.leadercontact == contact, Group.leadername == name, Group.cancel_yn == 0, or_(Group.camp_idx == Camp.get_idx('youth'))).one()
                member_list = Member.get_list(group.camp_idx, group_idx=group.idx)
                group_name = group.name
            except NoResultFound:
                return render_template('youth/room-check-result.html', msg=u'접수된 신청 정보가 없습니다^^ 이름과 연락처를 확인해주세요', name=name)
            except MultipleResultsFound:
                return render_template('youtb/room-check-result.html', msg=u'중복신청자입니다. 로비의 숙소배치팀에 문의해주세요.', name=name)
        else:
            pass

        return render_template('youth/room-check-result.html', member_list=member_list, group_name=group_name)

    else:
        from core.forms import RoomCheckForm
        form = RoomCheckForm()
        return render_template('youth/room-check.html', form=form)
...
def reg_individual():
    ...
    return render_template('youth/individual/add.html', camp='youth', campidx=campidx, area_list=area_list, group_yn=False)
...
def edit_individual():
    ...
    area_list = getAreaList('youth')  # form에 들어갈 지부 목록
    date_select_list = getDateSelectList()  # form 생년월일에 들어갈 날자 목록
    return render_template('youth/individual/edit.html', camp='youth', campidx=campidx, member=member, membership=member['membership'], area_list=area_list, date_select_list=date_select_list, group_yn=False)
...
def member_add():
    ...
    date_select_list = getDateSelectList()  # form 생년월일에 들어갈 날자 목록
    group = getGroupData(idx)
    return render_template('youth/individual/add.html', camp='youth', campidx=campidx, date_select_list=date_select_list, group_yn=True, group=group)
...
def member_edit(member_idx):
    ...
    date_select_list = getDateSelectList()  # form 생년월일에 들어갈 날자 목록
    group = getGroupData(idx)
    return render_template('youth/individual/edit.html', camp='cmc', campidx=campidx, member=member, membership=member['membership'], area_list=area_list, date_select_list=date_select_list, group_yn=True, group=group)
