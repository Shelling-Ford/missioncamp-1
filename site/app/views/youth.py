...
def room_check():
    if request.method == 'POST':
        ...
        if logintype == u'개인':
            try:
                member = db.db_session.query(Member).filter(Member.contact == contact, Member.name == name, Member.cancel_yn == 0, or_(Member.camp_idx == Camp.get_idx('youth'))).one()
                member_list = [member]
                group_name = None
            ...
            try:
                group = db.db_session.query(Group).filter(Group.leadercontact == contact, Group.leadername == name, Group.cancel_yn == 0, or_(Group.camp_idx == Camp.get_idx('youth'))).one()
